            ...
            {trip.isJointTrip && trip.jointPassengers && trip.jointPassengers.length > 0 ? (
              ...
              <div className="pt-3 mt-3 border-t border-slate-700/50">
                ...
                <div className="flex justify-between bg-slate-800/80 px-3 py-2 rounded items-center border border-slate-700">
                  <span className="text-[11px] text-slate-400 font-medium">Booked By: </span><span className="text-sm font-medium text-slate-200 mr-2">{trip.passengerName || 'Unknown User'}</span>{trip.nominatedName && <div className="mt-2 pt-2 border-t border-slate-700/50 flex justify-between items-center w-full"><span className="text-[10px] text-orange-400 font-bold uppercase tracking-wider">Nominated Traveler:</span><span className="text-sm font-bold text-orange-400">{trip.nominatedName} {trip.nominatedEmail && <span className="text-[10px] font-mono text-slate-400 font-normal">({trip.nominatedEmail})</span>}</span></div>}
